# [第17章 神兵利器-optimizer trace表的神器功效](https://relph1119.github.io/mysql-learning-notes/#/mysql/17-神兵利器-optimizer trace的神器功效?id=第17章-神兵利器-optimizer-trace表的神器功效)
...
```
小贴士：one_line的值是控制输出格式的，如果为on那么所有输出都将在一行中展示，不适合人阅读，所以我们就保持其默认值为off吧。Copy to clipboardErrorCopied
```
...
Query OK, 0 rows affected (0.00 sec)Copy to clipboardErrorCopied
```
...
SET optimizer_trace="enabled=off";Copy to clipboardErrorCopied
```
...
SELECT * FROM information_schema.OPTIMIZER_TRACE\G    Copy to clipboardErrorCopied
```
