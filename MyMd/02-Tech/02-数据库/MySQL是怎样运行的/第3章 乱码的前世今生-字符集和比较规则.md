...
- 编码规则如下：
	...
	'B' -> 00000100 (十六进制：0x04)Copy to clipboardErrorCopied
	```
...
'cd' -> 无法表示，字符集xiaohaizi不包含字符'c'和'd'Copy to clipboardErrorCopied
```

### [比较规则简介](https://relph1119.github.io/mysql-learning-notes/#/mysql/03-乱码的前世今生-字符集和比较规则?id=比较规则简介)
...
### [一些重要的字符集](https://relph1119.github.io/mysql-learning-notes/#/mysql/03-乱码的前世今生-字符集和比较规则?id=一些重要的字符集)
...
- `ASCII`字符集
	...
	'M' ->  01001101（十六进制：0x4D，十进制：77）Copy to clipboardErrorCopied
	```
...
- `GB2312`字符集
	...
	```
	小贴士：我们怎么区分某个字节代表一个单独的字符还是代表某个字符的一部分呢？别忘了`ASCII`字符集只收录128个字符，使用0～127就可以表示全部字符，所以如果某个字节是在0～127之内的，就意味着一个字节代表一个单独的字符，否则就是两个字节代表一个单独的字符。Copy to clipboardErrorCopied
	```
...
- `utf8`字符集
	...
	'啊' ->  111001011001010110001010（十六进制：0xE5958A）Copy to clipboardErrorCopied
	```

	```
	小贴士：其实准确的说，utf8只是Unicode字符集的一种编码方案，Unicode字符集可以采用utf8、utf16、utf32这几种编码方案，utf8使用1～4个字节编码一个字符，utf16使用2个或4个字节编码一个字符，utf32使用4个字节编码一个字符。更详细的Unicode和其编码方案的知识不是本书的重点，大家上网查查。MySQL中并不区分字符集和编码方案的概念，所以后边介绍的时候把utf8、utf16、utf32都当作一种字符集对待。Copy to clipboardErrorCopied
	```
	...
	gb2312编码：1100111011010010 (2个字节，十六进制表示是：0xCED2)Copy to clipboardErrorCopied
	```

## [MySQL中支持的字符集和排序规则](https://relph1119.github.io/mysql-learning-notes/#/mysql/03-乱码的前世今生-字符集和比较规则?id=mysql中支持的字符集和排序规则)

### [MySQL中的utf8和utf8mb4](https://relph1119.github.io/mysql-learning-notes/#/mysql/03-乱码的前世今生-字符集和比较规则?id=mysql中的utf8和utf8mb4)
...
### [字符集的查看](https://relph1119.github.io/mysql-learning-notes/#/mysql/03-乱码的前世今生-字符集和比较规则?id=字符集的查看)

  `MySQL`支持好多好多种字符集，查看当前`MySQL`中支持的字符集可以用下面这个语句：

```
SHOW (CHARACTER SET|CHARSET) [LIKE 匹配的模式];Copy to clipboardErrorCopied
```
...
+----------+---------------------------------+---------------------+--------+
41 rows in set (0.01 sec)Copy to clipboardErrorCopied
```
...
### [比较规则的查看](https://relph1119.github.io/mysql-learning-notes/#/mysql/03-乱码的前世今生-字符集和比较规则?id=比较规则的查看)

  查看`MySQL`中支持的比较规则的命令如下：

```
SHOW COLLATION [LIKE 匹配的模式];Copy to clipboardErrorCopied
```
...
+--------------------------+---------+-----+---------+----------+---------+
27 rows in set (0.00 sec)Copy to clipboardErrorCopied
```
...
```markup
比如`utf8_general_ci`这个比较规则是以`ci`结尾的，说明不区分大小写。Copy to clipboardErrorCopied
```
...
## [字符集和比较规则的应用](https://relph1119.github.io/mysql-learning-notes/#/mysql/03-乱码的前世今生-字符集和比较规则?id=字符集和比较规则的应用)

### [各级别的字符集和比较规则](https://relph1119.github.io/mysql-learning-notes/#/mysql/03-乱码的前世今生-字符集和比较规则?id=各级别的字符集和比较规则)
...
#### [服务器级别](https://relph1119.github.io/mysql-learning-notes/#/mysql/03-乱码的前世今生-字符集和比较规则?id=服务器级别)
...
#### [数据库级别](https://relph1119.github.io/mysql-learning-notes/#/mysql/03-乱码的前世今生-字符集和比较规则?id=数据库级别)
...
ALTER DATABASE 数据库名
    [[DEFAULT] CHARACTER SET 字符集名称]
    [[DEFAULT] COLLATE 比较规则名称];Copy to clipboardErrorCopied
```
...
```
CREATE DATABASE 数据库名;Copy to clipboardErrorCopied
```

  这样的话，将使用服务器级别的字符集和比较规则作为数据库的字符集和比较规则。

#### [表级别](https://relph1119.github.io/mysql-learning-notes/#/mysql/03-乱码的前世今生-字符集和比较规则?id=表级别)
...
ALTER TABLE 表名
    [[DEFAULT] CHARACTER SET 字符集名称]
    [COLLATE 比较规则名称]Copy to clipboardErrorCopied
```
...
#### [列级别](https://relph1119.github.io/mysql-learning-notes/#/mysql/03-乱码的前世今生-字符集和比较规则?id=列级别)
...
```
ALTER TABLE t MODIFY col VARCHAR(10);Copy to clipboardErrorCopied
```

  那列`col`的字符集和编码将使用表`t`的字符集和比较规则，也就是`utf8`和`utf8_general_ci`。

```
小贴士：在转换列的字符集时需要注意，如果转换前列中存储的数据不能用转换后的字符集进行表示，就会发生错误。比方说原先列使用的字符集是utf8，列中存储了一些汉字，现在把列的字符集转换为ascii的话就会出错，因为ascii字符集并不能表示汉字字符。Copy to clipboardErrorCopied
```

#### [仅修改字符集或仅修改比较规则](https://relph1119.github.io/mysql-learning-notes/#/mysql/03-乱码的前世今生-字符集和比较规则?id=仅修改字符集或仅修改比较规则)
...
- 只修改比较规则，则字符集将变为修改后的比较规则对应的字符集。
	...
	#### [各级别字符集和比较规则小结](https://relph1119.github.io/mysql-learning-notes/#/mysql/03-乱码的前世今生-字符集和比较规则?id=各级别字符集和比较规则小结)
...
### [客户端和服务器通信中的字符集](https://relph1119.github.io/mysql-learning-notes/#/mysql/03-乱码的前世今生-字符集和比较规则?id=客户端和服务器通信中的字符集)

#### [编码和解码使用的字符集不一致的后果](https://relph1119.github.io/mysql-learning-notes/#/mysql/03-乱码的前世今生-字符集和比较规则?id=编码和解码使用的字符集不一致的后果)
...
#### [字符集转换的概念](https://relph1119.github.io/mysql-learning-notes/#/mysql/03-乱码的前世今生-字符集和比较规则?id=字符集转换的概念)
...
#### [MySQL中字符集的转换](https://relph1119.github.io/mysql-learning-notes/#/mysql/03-乱码的前世今生-字符集和比较规则?id=mysql中字符集的转换)
...
```
SELECT * FROM t WHERE s = '我';Copy to clipboardErrorCopied
```
...
1. 客户端发送请求所使用的字符集
	...
	- `Windows`使用的是`gbk`
		...
		```
		小贴士：如果你使用的是可视化工具，比如navicat之类的，这些工具可能会使用自定义的字符集来编码发送到服务器的字符串，而不采用操作系统默认的字符集（所以在学习的时候还是尽量用黑框框）。Copy to clipboardErrorCopied
		```
...
3. 因为表`t`的列`col`采用的是`gbk`字符集，与`character_set_connection`一致，所以直接到列中找字节值为`0xCED2`的记录，最后找到了一条记录。
	...
	```
	小贴士：如果某个列使用的字符集和character_set_connection代表的字符集不一致的话，还需要进行一次字符集转换。Copy to clipboardErrorCopied
	```
...
![img](https://relph1119.github.io/mysql-learning-notes/images/03-01.png)
...
  知道了在`MySQL`中从发送请求到返回结果过程里发生的各种字符集转换，但是为什么要转来转去的呢？不晕么？
...
SET character_set_results = 字符集名;Copy to clipboardErrorCopied
```
...
mysql>Copy to clipboardErrorCopied
小贴士：如果你使用的是Windows系统，那应该设置成gbk。Copy to clipboardErrorCopied
```
...
```
[client]
default-character-set=utf8Copy to clipboardErrorCopied
```

  它起到的效果和执行一遍`SET NAMES utf8`是一样一样的，都会将那三个系统变量的值设置成`utf8`。

### [比较规则的应用](https://relph1119.github.io/mysql-learning-notes/#/mysql/03-乱码的前世今生-字符集和比较规则?id=比较规则的应用)
...
| 我   |
+------+
5 rows in set (0.00 sec)Copy to clipboardErrorCopied
```
...
Records: 5  Duplicates: 0  Warnings: 0Copy to clipboardErrorCopied
```
...
'a' -> 97 （十进制）
'b' -> 98 （十进制）
'我' -> 25105 （十进制）Copy to clipboardErrorCopied
```
