# [第9章 存放页的大池子-InnoDB的表空间](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=第9章-存放页的大池子-innodb的表空间)
...
## [回忆一些旧知识](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=回忆一些旧知识)

### [页类型](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=页类型)
...
### [页通用部分](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=页通用部分)

  我们前面说过数据页，也就是`INDEX`类型的页由7个部分组成，其中的两个部分是所有类型的页都通用的。当然我不能寄希望于你把我说的话都记住，所以在这里重新强调一遍，任何类型的页都有下面这种通用的结构：

![img](https://relph1119.github.io/mysql-learning-notes/images/09-01.png)
...
## [独立表空间结构](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=独立表空间结构)

  我们知道`InnoDB`支持许多种类型的表空间，本章重点关注独立表空间和系统表空间的结构。它们的结构比较相似，但是由于系统表空间中额外包含了一些关于整个系统的信息，所以我们先挑简单一点的独立表空间来介绍，稍后再说系统表空间的结构。

### [区（extent）的概念](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=区（extent）的概念)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/09-02.png)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/09-03.png)
...
### [段（segment）的概念](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=段（segment）的概念)
...
### [区的分类](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=区的分类)
...
```
小贴士：如果把表空间比作是一个集团军，段就相当于师，区就相当于团。一般的团都是隶属于某个师的，就像是处于`FSEG`的区全都隶属于某个段，而处于`FREE`、`FREE_FRAG`以及`FULL_FRAG`这三种状态的区却直接隶属于表空间，就像独立团直接听命于军部一样。Copy to clipboardErrorCopied
```
...
![img](https://relph1119.github.io/mysql-learning-notes/images/09-04.png)
...
- `List Node`（12字节）
	...
	![img](https://relph1119.github.io/mysql-learning-notes/images/09-05.png)
...
#### [XDES Entry链表](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=xdes-entry链表)
...
- 当段中数据已经占满了32个零散的页后，就直接申请完整的区来插入数据了。
	...
	- `FULL`链表：同一个段中，已经没有空闲空间的区对应的`XDES Entry`结构会被加入到这个链表。
		...
		CREATE TABLE t (
		  ...
		  PRIMARY KEY (c1),
		  KEY idx_c2 (c2)
		)ENGINE=InnoDB;Copy to clipboardErrorCopied
		```
...
#### [链表基节点](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=链表基节点)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/09-06.png)
...
#### [链表小结](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=链表小结)
...
### [段的结构](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=段的结构)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/09-07.png)
...
### [各类型页详细情况](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=各类型页详细情况)
...
#### [`FSP_HDR`类型](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=fsp_hdr类型)

  首先看第一个组的第一个页，当然也是表空间的第一个页，页号为`0`。这个页的类型是`FSP_HDR`，它存储了表空间的一些整体属性以及第一个组内256个区的对应的`XDES Entry`结构，直接看这个类型的页的示意图：

![img](https://relph1119.github.io/mysql-learning-notes/images/09-08.png)
...
##### [File Space Header部分](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=file-space-header部分)

  从名字就可以看出来，这个部分是用来存储表空间的一些整体属性的，废话少说，看图：

![img](https://relph1119.github.io/mysql-learning-notes/images/09-09.png)
...
- `Space Flags`
	...
	```
	小贴士：不同MySQL版本里 SPACE_FLAGS 代表的属性可能有些差异，我们这里列举的是5.7.21版本的。不过大家现在不必深究它们的意思，因为我们一旦把这些概念 展开，就需要非常大的篇幅，主要怕大家受不了。我们还是先挑重要的看，把主要的表空间结构了解完，这些 SPACE_FLAGS 里的属性的细节就暂时不深究了。Copy to clipboardErrorCopied
	```
...
##### [XDES Entry部分](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=xdes-entry部分)
...
#### [`XDES`类型](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=xdes类型)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/09-10.png)

  与`FSP_HDR`类型的页对比，除了少了`File Space Header`部分之外，也就是除了少了记录表空间整体属性的部分之外，其余的部分是一样一样的。由于我们上面介绍的已经够仔细了，对于`XDES`类型的页也就不重复介绍了。

#### [`IBUF_BITMAP`类型](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=ibuf_bitmap类型)
...
#### [`INODE`类型](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=inode类型)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/09-11.png)
...
### [Segment Header 结构的运用](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=segment-header-结构的运用)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/09-12.png)
...
### [真实表空间对应的文件大小](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=真实表空间对应的文件大小)
...
## [系统表空间](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=系统表空间)
...
### [系统表空间的整体结构](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=系统表空间的整体结构)

  系统表空间与独立表空间的一个非常明显的不同之处就是在表空间开头有许多记录整个系统属性的页，如图：

![img](https://relph1119.github.io/mysql-learning-notes/images/09-13.png)
...
#### [InnoDB数据字典](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=innodb数据字典)
...
  这些系统表也被称为`数据字典`，它们都是以`B+`树的形式保存在系统表空间的某些页中，其中`SYS_TABLES`、`SYS_COLUMNS`、`SYS_INDEXES`、`SYS_FIELDS`这四个表尤其重要，称之为基本系统表（basic system tables），我们先看看这4个表的结构：

##### [SYS_TABLES表](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=sys_tables表)
...
##### [SYS_COLUMNS表](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=sys_columns表)
...
##### [SYS_INDEXES表](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=sys_indexes表)
...
##### [SYS_FIELDS表](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=sys_fields表)
...
##### [Data Dictionary Header页](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=data-dictionary-header页)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/09-14.png)
...
##### [information_schema系统数据库](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=information_schema系统数据库)
...
+--------------------------------------------+
10 rows in set (0.00 sec)Copy to clipboardErrorCopied
```
...
### [总结图](https://relph1119.github.io/mysql-learning-notes/#/mysql/09-存放页的大池子-InnoDB的表空间?id=总结图)

  小册微信交流群2群中一个昵称为`think`同学非常有心的为表空间画了一个全局图，希望能对各位有帮助（这种学习态度实在让我感动😹）：

![img](https://relph1119.github.io/mysql-learning-notes/images/09-15.png)