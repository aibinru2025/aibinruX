# [第23章 后悔了怎么办-undo日志（下）](https://relph1119.github.io/mysql-learning-notes/#/mysql/23-后悔了怎么办-undo日志（下）?id=第23章-后悔了怎么办-undo日志（下）)
...
## [通用链表结构](https://relph1119.github.io/mysql-learning-notes/#/mysql/23-后悔了怎么办-undo日志（下）?id=通用链表结构)

  在写入`undo日志`的过程中会使用到多个链表，很多链表都有同样的节点结构，如图所示：

![img](https://relph1119.github.io/mysql-learning-notes/images/23-01.png)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/23-02.png)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/23-03.png)

```
小贴士：上述链表结构我们在前面的文章中频频提到，尤其是在表空间那一章重点描述过，不过我不敢奢求大家都记住了，所以在这里又强调一遍，希望大家不要嫌我烦，我只是怕大家忘了学习后续内容吃力而已～Copy to clipboardErrorCopied
```

## [FIL_PAGE_UNDO_LOG页面](https://relph1119.github.io/mysql-learning-notes/#/mysql/23-后悔了怎么办-undo日志（下）?id=fil_page_undo_log页面)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/23-04.png)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/23-05.png)
...
- `TRX_UNDO_PAGE_TYPE`：本页面准备存储什么种类的`undo日志`。
	...
	- `TRX_UNDO_UPDATE`（使用十进制`2`表示），除了类型为`TRX_UNDO_INSERT_REC`的`undo日志`，其他类型的`undo日志`都属于这个大类，比如我们前面说的`TRX_UNDO_DEL_MARK_REC`、`TRX_UNDO_UPD_EXIST_REC`什么的，一般由`DELETE`、`UPDATE`语句产生的`undo日志`属于这个大类。
		...
		```
		小贴士：之所以把undo日志分成两个大类，是因为类型为TRX_UNDO_INSERT_REC的undo日志在事务提交后可以直接删除掉，而其他类型的undo日志还需要为所谓的MVCC服务，不能直接删除掉，对它们的处理需要区别对待。当然，如果你看这段话迷迷糊糊的话，那就不需要再看一遍了，现在只需要知道undo日志分为2个大类就好了，更详细的东西我们后边会仔细介绍的。Copy to clipboardErrorCopied
		```
...
- `TRX_UNDO_PAGE_FREE`：与上面的`TRX_UNDO_PAGE_START`对应，表示当前页面中存储的最后一条`undo`日志结束时的偏移量，或者说从这个位置开始，可以继续写入新的`undo日志`。
	...
	![img](https://relph1119.github.io/mysql-learning-notes/images/23-06.png)
...
- `TRX_UNDO_PAGE_NODE`：代表一个`List Node`结构（链表的普通节点，我们上面刚说的）。
	...
	## [Undo页面链表](https://relph1119.github.io/mysql-learning-notes/#/mysql/23-后悔了怎么办-undo日志（下）?id=undo页面链表)

### [单个事务中的Undo页面链表](https://relph1119.github.io/mysql-learning-notes/#/mysql/23-后悔了怎么办-undo日志（下）?id=单个事务中的undo页面链表)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/23-07.png)
...
### [多个事务中的Undo页面链表](https://relph1119.github.io/mysql-learning-notes/#/mysql/23-后悔了怎么办-undo日志（下）?id=多个事务中的undo页面链表)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/23-10.png)

  如果有更多的事务，那就意味着可能会产生更多的`Undo页面`链表。

## [undo日志具体写入过程](https://relph1119.github.io/mysql-learning-notes/#/mysql/23-后悔了怎么办-undo日志（下）?id=undo日志具体写入过程)

### [段（Segment）的概念](https://relph1119.github.io/mysql-learning-notes/#/mysql/23-后悔了怎么办-undo日志（下）?id=段（segment）的概念)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/23-11.png)
...
```
小贴士：这部分关于段的各种概念我们在表空间那一章中都有详细解释，在这里重提一下只是为了唤醒大家沉睡的记忆，如果有任何不清楚的地方可以再次跳回表空间的那一章仔细读一下。Copy to clipboardErrorCopied
```

### [Undo Log Segment Header](https://relph1119.github.io/mysql-learning-notes/#/mysql/23-后悔了怎么办-undo日志（下）?id=undo-log-segment-header)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/23-12.png)

  可以看到这个`Undo`链表的第一个页面比普通页面多了个`Undo Log Segment Header`，我们来看一下它的结构：

![img](https://relph1119.github.io/mysql-learning-notes/images/23-13.png)
...
- `TRX_UNDO_STATE`：本`Undo页面`链表处在什么状态。
	...
	- `TRX_UNDO_PREPARED`：包含处于`PREPARE`阶段的事务产生的`undo日志`。
		...
		```
		小贴士：Undo页面链表什么时候会被重用，怎么重用我们之后会详细说的。事务的PREPARE阶段是在所谓的分布式事务中才出现的，本书中不会介绍更多关于分布式事务的事情，所以大家目前忽略这个状态就好了。Copy to clipboardErrorCopied
		```

- `TRX_UNDO_LAST_LOG`：本`Undo页面`链表中最后一个`Undo Log Header`的位置。

	```
	小贴士：关于什么是Undo Log Header，我们稍后马上介绍。Copy to clipboardErrorCopied
	```
...
### [Undo Log Header](https://relph1119.github.io/mysql-learning-notes/#/mysql/23-后悔了怎么办-undo日志（下）?id=undo-log-header)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/23-14.png)

这个`Undo Log Header`具体的结构如下：

![img](https://relph1119.github.io/mysql-learning-notes/images/23-15.png)
...
- `TRX_UNDO_XID_EXISTS`：本组`undo日志`是否包含XID信息。
	...
	```
	小贴士：本书不会讲述更多关于XID是个什么东东，有兴趣的同学可以到搜索引擎或者文档中搜一搜。Copy to clipboardErrorCopied
	```
...
- `TRX_UNDO_PREV_LOG`：上一组的`undo日志`在页面中开始的偏移量。
	...
	```
	小贴士：一般来说一个Undo页面链表只存储一个事务执行过程中产生的一组undo日志，但是在某些情况下，可能会在一个事务提交之后，之后开启的事务重复利用这个Undo页面链表，这样就会导致一个Undo页面中可能存放多组Undo日志，TRX_UNDO_NEXT_LOG和TRX_UNDO_PREV_LOG就是用来标记下一组和上一组undo日志在页面中的偏移量的。关于什么时候重用Undo页面链表，怎么重用这个链表我们稍后会详细说明的，现在先理解TRX_UNDO_NEXT_LOG和TRX_UNDO_PREV_LOG这两个属性的意思就好了。Copy to clipboardErrorCopied
	```

- `TRX_UNDO_HISTORY_NODE`：一个12字节的`List Node`结构，代表一个称之为`History`链表的节点。

	```
	小贴士：关于History链表我们后边会格外详细的介绍，现在先不用管。Copy to clipboardErrorCopied
	```

### [小结](https://relph1119.github.io/mysql-learning-notes/#/mysql/23-后悔了怎么办-undo日志（下）?id=小结)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/23-16.png)

## [重用Undo页面](https://relph1119.github.io/mysql-learning-notes/#/mysql/23-后悔了怎么办-undo日志（下）?id=重用undo页面)
...
- insert undo链表
	...
	![img](https://relph1119.github.io/mysql-learning-notes/images/23-17.png)
	...
	```
	小贴士：当然，在重用Undo页面链表写入新的一组undo日志时，不仅会写入新的Undo Log Header，还会适当调整Undo Page Header、Undo Log Segment Header、Undo Log Header中的一些属性，比如TRX_UNDO_PAGE_START、TRX_UNDO_PAGE_FREE等等等等，这些我们就不具体介绍了。Copy to clipboardErrorCopied
	```
...
- update undo链表
	...
	![img](https://relph1119.github.io/mysql-learning-notes/images/23-18.png)

## [回滚段](https://relph1119.github.io/mysql-learning-notes/#/mysql/23-后悔了怎么办-undo日志（下）?id=回滚段)

### [回滚段的概念](https://relph1119.github.io/mysql-learning-notes/#/mysql/23-后悔了怎么办-undo日志（下）?id=回滚段的概念)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/23-19.png)
...
- `TRX_RSEG_MAX_SIZE`：本`Rollback Segment`中管理的所有`Undo页面`链表中的`Undo页面`数量之和的最大值。换句话说，本`Rollback Segment`中所有`Undo页面`链表中的`Undo页面`数量之和不能超过`TRX_RSEG_MAX_SIZE`代表的值。
	...
	```
	小贴士：无限大其实也只是个夸张的说法，4个字节能表示最大的数也就是0xFFFFFFFF，但是我们之后会看到，0xFFFFFFFF这个数有特殊用途，所以实际上TRX_RSEG_MAX_SIZE的值为0xFFFFFFFE。Copy to clipboardErrorCopied
	```
...
- `TRX_RSEG_HISTORY`：`History`链表的基节点。
	...
	```
	小贴士：History链表后边讲，稍安勿躁。Copy to clipboardErrorCopied
	```
...
### [从回滚段中申请Undo页面链表](https://relph1119.github.io/mysql-learning-notes/#/mysql/23-后悔了怎么办-undo日志（下）?id=从回滚段中申请undo页面链表)
...
```
Too many active concurrent transactionsCopy to clipboardErrorCopied
```
...
- 如果该`undo slot`指向的`Undo页面`链表不符合被重用的条件，那么针对该`undo slot`对应的`Undo页面`链表类型不同，也会有不同的处理：
	...
	- 如果对应的`Undo页面`链表是`update undo链表`，则该`Undo页面`链表的`TRX_UNDO_STATE`属性会被设置为`TRX_UNDO_TO_PRUGE`，则会将该`undo slot`的值设置为`FIL_NULL`，然后将本次事务写入的一组`undo`日志放到所谓的`History链表`中（需要注意的是，这里并不会将`Undo页面`链表对应的段给释放掉，因为这些`undo`日志还有用呢～）。
		...
		```
		小贴士：更多关于History链表的事我们稍后再说，稍安勿躁。Copy to clipboardErrorCopied
		```

		### [多个回滚段](https://relph1119.github.io/mysql-learning-notes/#/mysql/23-后悔了怎么办-undo日志（下）?id=多个回滚段)
...
```
小贴士：只读事务并不需要分配Undo页面链表，MySQL 5.7中所有刚开启的事务默认都是只读事务，只有在事务执行过程中对记录做了某些改动时才会被升级为读写事务。Copy to clipboardErrorCopied
```
...
![img](https://relph1119.github.io/mysql-learning-notes/images/23-20.png)

  每个8字节的格子的构造就像这样：

![img](https://relph1119.github.io/mysql-learning-notes/images/23-21.png)
...
### [回滚段的分类](https://relph1119.github.io/mysql-learning-notes/#/mysql/23-后悔了怎么办-undo日志（下）?id=回滚段的分类)
...
```
小贴士：实际上在MySQL 5.7.21这个版本中，如果我们仅仅对普通表的记录做了改动，那么只会为该事务分配针对普通表的回滚段，不分配针对临时表的回滚段。但是如果我们仅仅对临时表的记录做了改动，那么既会为该事务分配针对普通表的回滚段，又会为其分配针对临时表的回滚段（不过分配了回滚段并不会立即分配undo slot，只有在真正需要Undo页面链表时才会去分配回滚段中的undo slot）。Copy to clipboardErrorCopied
```

### [为事务分配Undo页面链表详细过程](https://relph1119.github.io/mysql-learning-notes/#/mysql/23-后悔了怎么办-undo日志（下）?id=为事务分配undo页面链表详细过程)
...
## [回滚段相关配置](https://relph1119.github.io/mysql-learning-notes/#/mysql/23-后悔了怎么办-undo日志（下）?id=回滚段相关配置)

### [配置回滚段数量](https://relph1119.github.io/mysql-learning-notes/#/mysql/23-后悔了怎么办-undo日志（下）?id=配置回滚段数量)
...
### [配置undo表空间](https://relph1119.github.io/mysql-learning-notes/#/mysql/23-后悔了怎么办-undo日志（下）?id=配置undo表空间)
...
```
小贴士：如果我们在系统初始化的时候指定了创建了undo表空间，那么系统表空间中的第0号回滚段将处于不可用状态。Copy to clipboardErrorCopied
```
