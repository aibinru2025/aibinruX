...
## [不同类型的页简介](https://relph1119.github.io/mysql-learning-notes/#/mysql/05-盛放记录的大盒子-InnoDB数据页结构?id=不同类型的页简介)
...
## [数据页结构的快速浏览](https://relph1119.github.io/mysql-learning-notes/#/mysql/05-盛放记录的大盒子-InnoDB数据页结构?id=数据页结构的快速浏览)

  数据页代表的这块`16KB`大小的存储空间可以被划分为多个部分，不同部分有不同的功能，各个部分如图所示：

![img](https://relph1119.github.io/mysql-learning-notes/images/05-01.png)
...
```
小贴士：我们接下来并不打算按照页中各个部分的出现顺序来依次介绍它们，因为各个部分中会出现很多大家目前不理解的概念，这会打击各位读文章的信心与兴趣，希望各位能接受这种拍摄手法～Copy to clipboardErrorCopied
```

## [记录在页中的存储](https://relph1119.github.io/mysql-learning-notes/#/mysql/05-盛放记录的大盒子-InnoDB数据页结构?id=记录在页中的存储)
...
### [记录头信息的秘密](https://relph1119.github.io/mysql-learning-notes/#/mysql/05-盛放记录的大盒子-InnoDB数据页结构?id=记录头信息的秘密)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/05-04.png)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/05-05.png)
...
- `delete_mask`
	...
	```
	小贴士：将这个delete_mask位设置为1和将被删除的记录加入到垃圾链表中其实是两个阶段，我们后边在介绍事务的时候会详细介绍删除操作的详细过程，稍安勿躁。Copy to clipboardErrorCopied
	```
...
- `heap_no`
	...
	```
	小贴士：请注意我强调了对于`一条完整的记录`来说，比较记录的大小就相当于比的是主键的大小。后边我们还会介绍只存储一条记录的部分列的情况，敬请期待～Copy to clipboardErrorCopied
	```

	  但是不管我们向`页`中插入了多少自己的记录，设计`InnoDB`的大佬们都规定他们定义的两条伪记录分别为最小记录与最大记录。这两条记录的构造十分简单，都是由5字节大小的`记录头信息`和8字节大小的一个固定的部分组成的，如图所示

	![img](https://relph1119.github.io/mysql-learning-notes/images/05-06.png)

	  由于这两条记录不是我们自己定义的记录，所以它们并不存放在`页`的`User Records`部分，他们被单独放在一个称为`Infimum + Supremum`的部分，如图所示：

	![img](https://relph1119.github.io/mysql-learning-notes/images/05-07.png)
...
- `next_record`
	...
	![img](https://relph1119.github.io/mysql-learning-notes/images/05-08.png)
	...
	- 还有一点你可能忽略了，就是`最大记录`的`n_owned`值从`5`变成了`4`，关于这一点的变化我们稍后会详细说明的。
		...
		```
		小贴士：你会不会觉得next_record这个指针有点儿怪，为什么要指向记录头信息和真实数据之间的位置呢？为什么不干脆指向整条记录的开头位置，也就是记录的额外信息开头的位置呢？因为这个位置刚刚好，向左读取就是记录头信息，向右读取就是真实数据。我们前面还说过变长字段长度列表、NULL值列表中的信息都是逆序存放，这样可以使记录中位置靠前的字段和它们对应的字段长度信息在内存中的距离更近，可能会提高高速缓存的命中率。当然如果你看不懂这句话的话就不要勉强了，果断跳过～Copy to clipboardErrorCopied
		```
...
![img](https://relph1119.github.io/mysql-learning-notes/images/05-10.png)

  从图中可以看到，`InnoDB`并没有因为新记录的插入而为它申请新的存储空间，而是直接复用了原来被删除记录的存储空间。

```
小贴士：当数据页中存在多条被删除掉的记录时，这些记录的next_record属性将会把这些被删除掉的记录组成一个垃圾链表，以备之后重用这部分存储空间。Copy to clipboardErrorCopied
```

## [Page Directory（页目录）](https://relph1119.github.io/mysql-learning-notes/#/mysql/05-盛放记录的大盒子-InnoDB数据页结构?id=page-directory（页目录）)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/05-11.png)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/05-12.png)
...
Records: 12  Duplicates: 0  Warnings: 0Copy to clipboardErrorCopied
```
...
```
小贴士：如果你不知道二分法是个什么东西，找个基础算法书看看吧。什么？算法书写的看不懂？等我～Copy to clipboardErrorCopied
```

## [Page Header（页面头部）](https://relph1119.github.io/mysql-learning-notes/#/mysql/05-盛放记录的大盒子-InnoDB数据页结构?id=page-header（页面头部）)
...
小贴士：说到这个有些东西后边我们学过后回头看就很清晰的事儿不禁让我想到了乔布斯在斯坦福大学的演讲，摆一下原文：  
    ...
    上面这段话纯属心血来潮写的，大意是坚持做自己喜欢的事儿，你在做的时候可能并不能搞清楚这些事儿对自己之后的人生有什么影响，但当你一路走来回头看时，一切都是那么清晰，就像是命中注定的一样。上述内容跟MySQL毫无干系，请忽略～  Copy to clipboardErrorCopied
```

## [File Header（文件头部）](https://relph1119.github.io/mysql-learning-notes/#/mysql/05-盛放记录的大盒子-InnoDB数据页结构?id=file-header（文件头部）)
...
- `FIL_PAGE_PREV`和`FIL_PAGE_NEXT`
	...
	![img](https://relph1119.github.io/mysql-learning-notes/images/05-15.png)

	  关于`File Header`的其他属性我们暂时用不到，等用到的时候再提～

## [File Trailer](https://relph1119.github.io/mysql-learning-notes/#/mysql/05-盛放记录的大盒子-InnoDB数据页结构?id=file-trailer)
...
## [总结](https://relph1119.github.io/mysql-learning-notes/#/mysql/05-盛放记录的大盒子-InnoDB数据页结构?id=总结)
