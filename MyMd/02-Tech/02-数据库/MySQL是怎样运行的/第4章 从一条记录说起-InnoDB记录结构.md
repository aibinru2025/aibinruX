# [第4章 从一条记录说起-InnoDB记录结构](https://relph1119.github.io/mysql-learning-notes/#/mysql/04-从一条记录说起-InnoDB记录结构?id=第4章-从一条记录说起-innodb记录结构)

## [准备工作](https://relph1119.github.io/mysql-learning-notes/#/mysql/04-从一条记录说起-InnoDB记录结构?id=准备工作)
...
## [InnoDB页简介](https://relph1119.github.io/mysql-learning-notes/#/mysql/04-从一条记录说起-InnoDB记录结构?id=innodb页简介)
...
## [InnoDB行格式](https://relph1119.github.io/mysql-learning-notes/#/mysql/04-从一条记录说起-InnoDB记录结构?id=innodb行格式)
...
### [指定行格式的语法](https://relph1119.github.io/mysql-learning-notes/#/mysql/04-从一条记录说起-InnoDB记录结构?id=指定行格式的语法)
...
### [COMPACT行格式](https://relph1119.github.io/mysql-learning-notes/#/mysql/04-从一条记录说起-InnoDB记录结构?id=compact行格式)
...
#### [记录的额外信息](https://relph1119.github.io/mysql-learning-notes/#/mysql/04-从一条记录说起-InnoDB记录结构?id=记录的额外信息)

  这部分信息是服务器为了描述这条记录而不得不额外添加的一些信息，这些额外信息分为3类，分别是`变长字段长度列表`、`NULL值列表`和`记录头信息`，我们分别看一下。

##### [变长字段长度列表](https://relph1119.github.io/mysql-learning-notes/#/mysql/04-从一条记录说起-InnoDB记录结构?id=变长字段长度列表)
...
```
01 03 04 Copy to clipboardErrorCopied
```

  把这个字节串组成的`变长字段长度列表`填入上面的示意图中的效果就是：

![填入变长字段长度列表](https://relph1119.github.io/mysql-learning-notes/images/04-02.png)
...
- 如果`M×W <= 255`，那么使用1个字节来表示真正字符串占用的字节数。
	...
	```
	  就是说InnoDB在读记录的变长字段长度列表时先查看表结构，如果某个变长字段允许存储的最大字节数不大于255时，可以认为只使用1个字节来表示真正字符串占用的字节数。Copy to clipboardErrorCopied
	```
...
![填充完`变长字段长度列表`的两条记录的对比图](https://relph1119.github.io/mysql-learning-notes/images/04-03.png)

```
小贴士：并不是所有记录都有这个 变长字段长度列表 部分，比方说表中所有的列都不是变长的数据类型的话，这一部分就不需要有。Copy to clipboardErrorCopied
```

##### [NULL值列表](https://relph1119.github.io/mysql-learning-notes/#/mysql/04-从一条记录说起-InnoDB记录结构?id=null值列表)
...
2. 如果表中没有允许存储 ***NULL\*** 的列，则 *NULL值列表* 也不存在了，否则将每个允许存储`NULL`的列对应一个二进制位，二进制位按照列的顺序逆序排列，二进制位表示的意义如下：
	...
	- 二进制位的值为`0`时，代表该列的值不为`NULL`。
		...
		![record_format_demo表3值对应关系](https://relph1119.github.io/mysql-learning-notes/images/04-04.png)
...
3. `MySQL`规定`NULL值列表`必须用整数个字节的位表示，如果使用的二进制位个数不是整数个字节，则在字节的高位补`0`。
	...
	![img](https://relph1119.github.io/mysql-learning-notes/images/04-05.png)
...
- 对于第一条记录来说，`c1`、`c3`、`c4`这3个列的值都不为`NULL`，所以它们对应的二进制位都是`0`，画个图就是这样：
	...
	![img](https://relph1119.github.io/mysql-learning-notes/images/04-06.png)  所以第一条记录的`NULL值列表`用十六进制表示就是：`0x00`。

- 对于第二条记录来说，`c1`、`c3`、`c4`这3个列中`c3`和`c4`的值都为`NULL`，所以这3个列对应的二进制位的情况就是：

	![img](https://relph1119.github.io/mysql-learning-notes/images/04-07.png)

	  所以第二条记录的`NULL值列表`用十六进制表示就是：`0x06`。

  所以这两条记录在填充了`NULL值列表`后的示意图就是这样：

![img](https://relph1119.github.io/mysql-learning-notes/images/04-08.png)

##### [记录头信息](https://relph1119.github.io/mysql-learning-notes/#/mysql/04-从一条记录说起-InnoDB记录结构?id=记录头信息)

  除了`变长字段长度列表`、`NULL值列表`之外，还有一个用于描述记录的`记录头信息`，它是由固定的`5`个字节组成。`5`个字节也就是`40`个二进制位，不同的位代表不同的意思，如图：

![img](https://relph1119.github.io/mysql-learning-notes/images/04-09.png)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/04-10.png)

```
小贴士：再一次强调，大家如果看不懂记录头信息里各个位代表的概念千万别纠结，我们后边会说的～Copy to clipboardErrorCopied
```

#### [记录的真实数据](https://relph1119.github.io/mysql-learning-notes/#/mysql/04-从一条记录说起-InnoDB记录结构?id=记录的真实数据)
...
```
小贴士：实际上这几个列的真正名称其实是：DB_ROW_ID、DB_TRX_ID、DB_ROLL_PTR，我们为了美观才写成了row_id、transaction_id和roll_pointer。Copy to clipboardErrorCopied
```
...
![img](https://relph1119.github.io/mysql-learning-notes/images/04-11.png)
...
#### [CHAR(M)列的存储格式](https://relph1119.github.io/mysql-learning-notes/#/mysql/04-从一条记录说起-InnoDB记录结构?id=charm列的存储格式)

  `record_format_demo`表的`c1`、`c2`、`c4`列的类型是`VARCHAR(10)`，而`c3`列的类型是`CHAR(10)`，我们说在`Compact`行格式下只会把变长类型的列的长度逆序存到`变长字段长度列表`中，就像这样：

![img](https://relph1119.github.io/mysql-learning-notes/images/04-12.png)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/04-13.png)
...
### [Redundant行格式](https://relph1119.github.io/mysql-learning-notes/#/mysql/04-从一条记录说起-InnoDB记录结构?id=redundant行格式)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/04-14.png)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/04-15.png)
...
### [行溢出数据](https://relph1119.github.io/mysql-learning-notes/#/mysql/04-从一条记录说起-InnoDB记录结构?id=行溢出数据)

#### [VARCHAR(M)最多能存储的数据](https://relph1119.github.io/mysql-learning-notes/#/mysql/04-从一条记录说起-InnoDB记录结构?id=varcharm最多能存储的数据)
...
#### [记录中的数据太多产生的溢出](https://relph1119.github.io/mysql-learning-notes/#/mysql/04-从一条记录说起-InnoDB记录结构?id=记录中的数据太多产生的溢出)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/04-16.png)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/04-17.png)

  最后需要注意的是，不只是 ***VARCHAR(M)\*** 类型的列，其他的 ***TEXT\***、***BLOB*** 类型的列在存储数据非常多的时候也会发生`行溢出`。

#### [行溢出的临界点](https://relph1119.github.io/mysql-learning-notes/#/mysql/04-从一条记录说起-InnoDB记录结构?id=行溢出的临界点)
...
### [Dynamic和Compressed行格式](https://relph1119.github.io/mysql-learning-notes/#/mysql/04-从一条记录说起-InnoDB记录结构?id=dynamic和compressed行格式)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/04-18.png)

  `Compressed`行格式和`Dynamic`不同的一点是，`Compressed`行格式会采用压缩算法对页面进行压缩，以节省空间。

## [总结](https://relph1119.github.io/mysql-learning-notes/#/mysql/04-从一条记录说起-InnoDB记录结构?id=总结)
