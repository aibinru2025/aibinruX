# [第7章 好东西也得先学会怎么用-B+树索引的使用](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=第7章-好东西也得先学会怎么用-b树索引的使用)
...
## [索引的代价](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=索引的代价)
...
## [B+树索引适用的条件](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=b树索引适用的条件)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/07-01.png)
...
### [全值匹配](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=全值匹配)

  如果我们的搜索条件中的列和索引列一致的话，这种情况就称为全值匹配，比方说下面这个查找语句：

```
SELECT * FROM person_info WHERE name = 'Ashburn' AND birthday = '1990-09-27' AND phone_number = '15123983239';Copy to clipboardErrorCopied
```
...
```
SELECT * FROM person_info WHERE birthday = '1990-09-27' AND phone_number = '15123983239' AND name = 'Ashburn';Copy to clipboardErrorCopied
```

  答案是：没影响。`MySQL`有一个叫查询优化器的东东，会分析这些搜索条件并且按照可以使用的索引中列的顺序来决定先使用哪个搜索条件，后使用哪个搜索条件。我们后边儿会有专门的章节来介绍查询优化器，敬请期待。

### [匹配左边的列](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=匹配左边的列)

  其实在我们的搜索语句中也可以不用包含全部联合索引中的列，只包含左边的就行，比方说下面的查询语句：

```
SELECT * FROM person_info WHERE name = 'Ashburn';Copy to clipboardErrorCopied
```

  或者包含多个左边的列也行：

```
SELECT * FROM person_info WHERE name = 'Ashburn' AND birthday = '1990-09-27';Copy to clipboardErrorCopied
```

  那为什么搜索条件中必须出现左边的列才可以使用到这个`B+`树索引呢？比如下面的语句就用不到这个`B+`树索引么？

```
SELECT * FROM person_info WHERE birthday = '1990-09-27';Copy to clipboardErrorCopied
```
...
```
SELECT * FROM person_info WHERE name = 'Ashburn' AND phone_number = '15123983239';Copy to clipboardErrorCopied
```
...
### [匹配列前缀](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=匹配列前缀)
...
Barlow
...
BarlowCopy to clipboardErrorCopied
```
...
```
SELECT * FROM person_info WHERE name LIKE 'As%';Copy to clipboardErrorCopied
```
...
### [匹配范围值](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=匹配范围值)

  回头看我们`idx_name_birthday_phone_number`索引的`B+`树示意图，所有记录都是按照索引列的值从小到大的顺序排好序的，所以这极大的方便我们查找索引列的值在某个范围内的记录。比方说下面这个查询语句：

```
SELECT * FROM person_info WHERE name > 'Asa' AND name < 'Barlow';Copy to clipboardErrorCopied
```
...
```
SELECT * FROM person_info WHERE name > 'Asa' AND name < 'Barlow' AND birthday > '1980-01-01';Copy to clipboardErrorCopied
```
...
### [精确匹配某一列并范围匹配另外一列](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=精确匹配某一列并范围匹配另外一列)

  对于同一个联合索引来说，虽然对多个列都进行范围查找时只能用到最左边那个索引列，但是如果左边的列是精确查找，则右边的列可以进行范围查找，比方说这样：

```
SELECT * FROM person_info WHERE name = 'Ashburn' AND birthday > '1980-01-01' AND birthday < '2000-12-31' AND phone_number > '15100000000';Copy to clipboardErrorCopied
```
...
```
SELECT * FROM person_info WHERE name = 'Ashburn' AND birthday = '1980-01-01' AND phone_number > '15100000000';Copy to clipboardErrorCopied
```

### [用于排序](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=用于排序)
...
```
SELECT * FROM person_info ORDER BY name, birthday, phone_number LIMIT 10;Copy to clipboardErrorCopied
```
...
#### [使用联合索引进行排序注意事项](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=使用联合索引进行排序注意事项)
...
```
SELECT * FROM person_info WHERE name = 'A' ORDER BY birthday, phone_number LIMIT 10;Copy to clipboardErrorCopied
```

  这个查询能使用联合索引进行排序是因为`name`列的值相同的记录是按照`birthday`, `phone_number`排序的，说了好多遍了都。

#### [不可以使用索引进行排序的几种情况](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=不可以使用索引进行排序的几种情况)

##### [ASC、DESC混用](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=asc、desc混用)

  对于使用联合索引进行排序的场景，我们要求各个排序列的排序顺序是一致的，也就是要么各个列都是`ASC`规则排序，要么都是`DESC`规则排序。

```
小贴士：
ORDER BY子句后的列如果不加ASC或者DESC默认是按照ASC排序规则排序的，也就是升序排序的。Copy to clipboardErrorCopied
```
...
- `ORDER BY name DESC, birthday DESC LIMIT 10`
	...
	```
	SELECT * FROM person_info ORDER BY name, birthday DESC LIMIT 10;Copy to clipboardErrorCopied
	```
...
##### [WHERE子句中出现非排序使用到的索引列](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=where子句中出现非排序使用到的索引列)

  如果WHERE子句中出现了非排序使用到的索引列，那么排序依然是使用不到索引的，比方说这样：

```
SELECT * FROM person_info WHERE country = 'China' ORDER BY name LIMIT 10;Copy to clipboardErrorCopied
```
...
##### [排序列包含非同一个索引的列](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=排序列包含非同一个索引的列)

  有时候用来排序的多个列不是一个索引里的，这种情况也不能使用索引进行排序，比方说：

```
SELECT * FROM person_info ORDER BY name, country LIMIT 10;Copy to clipboardErrorCopied
```

`name`和`country`并不属于一个联合索引中的列，所以无法使用索引进行排序，至于为什么我就不想再介绍了，自己用前面的理论自己捋一捋把～

##### [排序列使用了复杂的表达式](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=排序列使用了复杂的表达式)

  要想使用索引进行排序操作，必须保证索引列是以单独列的形式出现，而不是修饰过的形式，比方说这样：

```
SELECT * FROM person_info ORDER BY UPPER(name) LIMIT 10;Copy to clipboardErrorCopied
```

  使用了`UPPER`函数修饰过的列就不是单独的列啦，这样就无法使用索引进行排序啦。

### [用于分组](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=用于分组)

  有时候我们为了方便统计表中的一些信息，会把表中的记录按照某些列进行分组。比如下面这个分组查询：

```
SELECT name, birthday, phone_number, COUNT(*) FROM person_info GROUP BY name, birthday, phone_numberCopy to clipboardErrorCopied
```
...
## [回表的代价](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=回表的代价)
...
```
SELECT * FROM person_info WHERE name > 'Asa' AND name < 'Barlow' LIMIT 10;Copy to clipboardErrorCopied
```
...
```
SELECT * FROM person_info ORDER BY name, birthday, phone_number;Copy to clipboardErrorCopied
```
...
### [覆盖索引](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=覆盖索引)

  为了彻底告别`回表`操作带来的性能损耗，我们建议：最好在查询列表里只包含索引列，比如这样：

```
SELECT name, birthday, phone_number FROM person_info  
    WHERE name > 'Asa' AND name < 'Barlow';Copy to clipboardErrorCopied
```
...
```
SELECT name, birthday, phone_number  FROM person_info   
    ORDER BY name, birthday, phone_number;Copy to clipboardErrorCopied
```
...
## [如何挑选索引](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=如何挑选索引)

  上面我们以`idx_name_birthday_phone_number`索引为例对索引的适用条件进行了详细的介绍，下面看一下我们在建立索引时或者编写查询语句时就应该注意的一些事项。

### [只为用于搜索、排序或分组的列创建索引](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=只为用于搜索、排序或分组的列创建索引)

  也就是说，只为出现在`WHERE`子句中的列、连接子句中的连接列，或者出现在`ORDER BY`或`GROUP BY`子句中的列创建索引。而出现在查询列表中的列就没必要建立索引了：

```
SELECT birthday, country FROM person_name WHERE name = 'Ashburn';Copy to clipboardErrorCopied
```

  像查询列表中的`birthday`、`country`这两个列就不需要建立索引，我们只需要为出现在`WHERE`子句中的`name`列创建索引就可以了。

### [考虑列的基数](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=考虑列的基数)
...
### [索引列的类型尽量小](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=索引列的类型尽量小)
...
### [索引字符串值的前缀](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=索引字符串值的前缀)
...
  我们前面儿说过索引列的字符串前缀其实也是排好序的，所以索引的设计者提出了个方案 --- 只对字符串的前几个字符进行索引也就是说在二级索引的记录中只保留字符串前几个字符。这样在查找记录时虽然不能精确的定位到记录的位置，但是能定位到相应前缀所在的位置，然后根据前缀相同的记录的主键值回表查询完整的字符串值，再对比就好了。这样只在`B+`树中存储字符串的前几个字符的编码，既节约空间，又减少了字符串的比较时间，还大概能解决排序的问题，何乐而不为，比方说我们在建表语句中只对`name`列的前10个字符进行索引可以这么写：
...
#### [索引列前缀对排序的影响](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=索引列前缀对排序的影响)

  如果使用了索引列前缀，比方说前面只把`name`列的前10个字符放到了二级索引中，下面这个查询可能就有点儿尴尬了：

```
SELECT * FROM person_info ORDER BY name LIMIT 10;Copy to clipboardErrorCopied
```

  因为二级索引中不包含完整的`name`列信息，所以无法对前十个字符相同，后边的字符不同的记录进行排序，也就是使用索引列前缀的方式无法支持使用索引排序，只好乖乖的用文件排序喽。

### [让索引列在比较表达式中单独出现](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=让索引列在比较表达式中单独出现)
...
### [主键插入顺序](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=主键插入顺序)
...
![img](https://relph1119.github.io/mysql-learning-notes/images/07-02.png)

  如果此时再插入一条主键值为`9`的记录，那它插入的位置就如下图：

![img](https://relph1119.github.io/mysql-learning-notes/images/07-03.png)
...
### [冗余和重复索引](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=冗余和重复索引)
...
## [总结](https://relph1119.github.io/mysql-learning-notes/#/mysql/07-好东西也得先学会怎么用-B+树索引的使用?id=总结)
